...
const Section2 = ({ videoCover, footer }) => {
  ...
  return (
    <div ref={container} className='w-full min-h-screen py-[5vw]'>
      {data.map((d, i) => (<div ref={(el) => wrappers.current[i] = el} key={i} className='wrap w-full sm:h-[235px] md:h-[80%] lg:h-[50vh] overflow-hidden  xl:h-[80%]  h-[80%] flex items-start justify-between relative'>
        <div className="line1t bg-white w-full h-[1px] absolute top-0 left-1/2 -translate-x-1/2"></div>
        {data.length - 1 === i && <div className="line1b bg-white w-full h-[1px] z-10 absolute bottom-0 left-0"></div>}
        ...
        <div className="card  sm:w-[50%]  md:w-[50%] w-[calc(100%/3)]  relative text-white">
          ...
          <div className='card-content scrollstyle  p-[3vw] py-[3vw] sm:h-[235px] md:h-[80%] lg:h-[80%] xl:h-[80%]  overflow-y-scroll overflow-hidden'>
